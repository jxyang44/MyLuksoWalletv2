...
const GetStarted = () => {
  ...
  useEffect(() => {
    animateOnEntry(imgRef, setPersAnimation, 1.2);
  }, [scrollHeight]);
  const navigate = useNavigate();
  return (
    <div
      className={`overflow-hidden bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 sm:grid sm:grid-cols-2 sm:items-center md:my-32 opacity-0 ${
        persAnimation && "animate-fadeInCenter"
      }`}
      ref={imgRef}>
      ...
      <img
        alt="Get Started"
        src={getstarted}
        className={`h-full w-full object-cover transition sm:h-[calc(100%_-_2rem)] sm:self-end sm:rounded-tl-[30px] md:h-[calc(100%_-_4rem)] md:rounded-tl-[60px]`}
      />
    </div>
  );
};
