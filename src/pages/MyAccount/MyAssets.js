...
import {Button_Shadow, NFTCard, TokenCardFull, Loading, Banner, LoginGraphic} from '../../components';
...
const MyAssets = ({LSP_7_8_or_9, LSP_5_10_or_12}) => {
    ...
    const {receivedAssets, ownedAssets, fetchAllAssets, fetchOwnedAssets, supportsInterface} = useAssetsContext();
   ...
   const handleLoadAssets = () =>{
    ...
    fetchAllAssets(currentAccount, LSPMapping[LSP_5_10_or_12].name).then(()=>{
      setAssets([]);
      receivedAssets.length && receivedAssets.forEach(asset => {
        supportsInterface(asset, LSPMapping[LSP_7_8_or_9].contract).then(res =>{
          if(res==true)
    ...
    <>
     <Banner 
        ...
        subtitle = {`${currentAccount ?`UP Address: ${currentAccount}` : "No Universal Profile address detected."}`}
              ...
              <div className="flex flex-wrap basis-1/3 gap-4 grid-cols-3">
                {assets.length ?
                  ...
                  {assets.map((asset, index) => {
                    return(
                        <>
                          {LSP_7_8_or_9==="LSP7" && <TokenCardFull key={index} assetAddress={asset} />}
                          {LSP_7_8_or_9==="LSP8" && <NFTCard key={index} assetAddress={asset} />}
                        </>
                        )
                      })
                    }
