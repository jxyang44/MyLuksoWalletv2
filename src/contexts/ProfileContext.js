...
export const ProfileProvider = ({ children }) => {
  ...
  const { setTheme, setUPColor, setUPTextColor, setThemeDefaults } =
    useStateContext(); //state variables for website themes, fetched from UP metadata
  ...
  const [profileJSONMetadata, setProfileJSONMetadata] =
    useState(defaultMetadata); // profile metadata - should always be in sync with JSON metadata on IPFS
  const [pendingProfileJSONMetadata, setPendingProfileJSONMetadata] =
    useState(defaultMetadata); // keeps track of pending profile metadata that the user makes locally, but hasn't committed to uploading to the blockchain
  ...
  async function fetchProfileMetadata(address) {
    ...
    if (profileData.value !== null) {
      ...
      swal(
        "Your account is now connected to MyLuksoWallet.",
      ...
      swal(
        "Your account is now connected to MyLuksoWallet.",
        "No initial profile metadata was found for this account.",
        "warning"
      );
    }
  }
  ...
  async function addNewPermission(addressFrom, addressTo, permissions) {
    if (!currentAccount)
      return swal("You are not connected to an account.", "", "warning");
    if (!addressFrom || !addressTo || !permissions)
      return swal("A required address was not provided.", "", "warning");
    try {
      swal(`Adding new permissions for ${addressTo} to ${addressFrom}...`, {
        button: false,
      });
      ...
      const myUniversalProfile = new web3Window.eth.Contract(
        UniversalProfileContract.abi,
        currentAccount
      );
      ...
      if (currentAccount === addressFrom) {
        ...
        executeViaKeyManager(
          ...
          `Please wait. Adding permissions via key manager...`,
          "Your permissions were added!"
        );
      }
    } catch (error) {
      console.log(error);
  ...
  async function updateExistingPermission(addressFrom, addressTo, permissions) {
    ...
    if (!addressFrom || !addressTo || !permissions)
      return swal("A required address was not provided.", "", "warning");
    try {
      swal(`Updating permissions for ${addressTo} on ${addressFrom}...`, {
        button: false,
      });
      ...
      if (currentAccount === addressFrom) {
        ...
        executeViaKeyManager(
          executePayload.encodeABI,
          `Please wait. Updating permissions via key manager...`,
  ...
  const executeViaKeyManager = async (
    functionABI,
    functionMessage,
    successMessage
  ) => {
    ...
    const runExecuteFunction = async (privateKey) => {
      try {
        ...
        const myKM = new web3Window.eth.Contract(
          LSP6Contract.abi,
          keyManagerAddress
        );
        swal(functionMessage, { button: false });
        return await myKM.methods
          .execute(abiPayload)
