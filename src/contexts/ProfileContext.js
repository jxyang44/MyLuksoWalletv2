...
export const ProfileProvider = ({ children }) => {
  ...
  async function fetchProfileMetadata(address) {
      ...
      setProfileJSONMetadata(current => ({...current, ...profileData.value.LSP3Profile}));
      setPendingProfileJSONMetadata(current => ({...current, ...profileData.value.LSP3Profile})); 
      setIsProfileLoaded(true);
      swal("Your Universal Profile is now connected to MyLuksoWallet.",`Welcome, ${profileData.value.LSP3Profile.name}!`,"success");
  ...
  return (
    <ProfileContext.Provider
      value={{
        defaultMetadata,
        isWalletConnected,
        connectProfile, connectProfileUsingPrivateKey,
        currentAccount, setCurrentAccount,
        profileJSONMetadata, setProfileJSONMetadata,
        pendingProfileJSONMetadata, setPendingProfileJSONMetadata,
        isProfileLoaded, setIsProfileLoaded,
        maxImageIndex,
