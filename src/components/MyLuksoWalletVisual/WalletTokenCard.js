  ...
  return (
    <div className="flex flex-col w-5/6 items-center justify-start my-2">
        <div className="w-5/6 h-[8vmin] ml-4 bg-slate-400 border-4 border-slate-900 rounded-t-3xl border-b-0 token-card-test flex flex-row justify-between">
            ...
            <div>
             {assetMetadata && <img className ="w-16" src={assetMetadata.icon[0].url.replace("ipfs://", IPFS_GATEWAY)}></img>}
            </div>
        </div>
        <div className="h-[1vmin] w-full bg-slate-600 border-b-[2px] border-b-slate-200 border-t-[2px] border-t-slate-800 z-100">
        </div>
    </div>
  )
}
