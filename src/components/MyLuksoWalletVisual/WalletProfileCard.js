...
import { Address, LYXBalanceFuncs } from "..";
                ...
                <div className="flex flex-col lg:gap-4 gap-1">
                  ...
                  {pendingProfileJSONMetadata.links && (
                    <div className="mr-4 w-11/12 break-words flex flex-col">
                      {pendingProfileJSONMetadata.links.map(item => {
                        return (
                          <a key={item} href={item.url} className="not-italic text-left text-blue-500 hover:text-blue-300" rel="noreferrer" target="_blank">
                            {item.title}
                          </a>
                        );
                      })}
