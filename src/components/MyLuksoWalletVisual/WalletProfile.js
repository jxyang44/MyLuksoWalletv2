...
const WalletProfile = () => {
  ...
  const {currentAccount, isProfileLoaded, pendingProfileJSONMetadata, setPendingProfileJSONMetadata, maxImageIndex } = useProfileContext();
  const {accountBalance} = useAssetsContext();
  return (
    <div className="flex justify-center p-1 h-full w-5/6">
    <div className={`relative p-3 justify-end items-start flex-col rounded-xl h-full aspect-[1.8] bg-slate-700 text-white ring-2`}>
      <div className="flex justify-between flex-col w-full h-full">
        <div className="flex justify-between items-start text-lg text-neutral-100 mix-blend-lighten">
          <div className="flex flex-col h-full w-3/4">
            <p className={"font-bold text-3xl mb-2 tracking-tight text-transparent bg-clip-text bg-gradient-to-tl from-white via-blue-500 to-white drop-shadow-3xl shadow-black hover:bg-gradient-to-tr"}>
            ...
            <div className='text-sm'>
              <div className="flex flex-col">
                ...
                <p className='mt-3 mr-4 w-11/12 break-words'>
                  {pendingProfileJSONMetadata.description.substring(0,80)} {pendingProfileJSONMetadata.description.length >= 80 && "..."}
                </p>
              </div>
            </div>
          </div>
          <div className={`relative w-3/12 flex justify-center items-center rounded-md border-2 border-slate-200`}>
              ...
              <img className="rounded" src={pendingProfileJSONMetadata.profileImage[maxImageIndex(pendingProfileJSONMetadata.profileImage,100)].url?.replace("ipfs://", IPFS_GATEWAY)}></img>}
          </div>
        </div>
