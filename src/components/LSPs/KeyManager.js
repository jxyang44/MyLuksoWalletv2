...
const KeyManager = () => {
  ...
  const checkPermissions = () => {
    const erc725 = new ERC725(LSP6Schema, currentAccount, web3.currentProvider);
  ...
  const handleGrantPermissions = () => {
    ...
    const privateKeyIsValid = privateKey => {
      //function to validate private key for extension
      const myUPExtension = web3.eth.accounts.wallet.add(privateKey);
      if (myUPExtension.address === currentAccount) return true;
      else {
        return false;
      }
    };
  ...
  return (
    <div className="flex flex-col gap-10">
      <Button_Shadow
        ...
        buttonFunc={checkPermissions}
        buttonColor={"bg-orange-800"}
        buttonTextColor={"text-emerald-200"}
      />
      <Button_Shadow
        buttonText={"Grant Permissions"}
        buttonFunc={handleGrantPermissions}
        buttonColor={"bg-emerald-800"}
