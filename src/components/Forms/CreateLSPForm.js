...
const imageLabel =
  " text-xs text-center cursor-pointer font-semibold text-white";
...
const CreateLSPForm = ({
  ...
  const handleCreateLSP7Contract = () => {
    const createTokenContract = async () => {
      try {
        console.log(
          "------------ deploying new LSP7 token contract ------------"
        );
        swal(
          "Deploying your LSP7Mintable token contract... (we pay for gas during the hackathon)",
    ...
    currentAccount
      ? createTokenContract().then((res) => {
          ...
          if (res && Number(formValues.mintAmount) > 0) {
            ...
            mintLSP7(
              res.LSP7DigitalAsset.address,
              formValues.mintAmount,
              currentAccount,
              LSP7MintableContract
            );
          }
  ...
  const handleCreateLSP8Contract = () => {
    const createTokenContract = async () => {
      try {
        console.log(
          "------------ deploying new LSP8 token contract ------------"
        );
        swal(
        ...
        return await lspFactory.LSP8IdentifiableDigitalAsset.deploy(
          ...
          },
          {
            ...
            onDeployEvents: {
              ...
              complete: async (contracts) => {
                const newAddress =
                  contracts.LSP8IdentifiableDigitalAsset.address;
                swal(
    ...
    currentAccount
      ? createTokenContract().then((res) => {
          ...
          if (res)
            mintLSP8(
              res.LSP8IdentifiableDigitalAsset.address,
              formValues.tokenID,
              currentAccount,
              LSP8MintableContract
            );
    ...
    return ({ target }) => {
      target.files.length &&
        setFormValues((current) => ({
          ...current,
          [name]: target.files[0],
          [name + "URL"]: URL.createObjectURL(target.files[0]),
        }));
    };
  };
  ...
  return (
    <FormContainer
      title={`Create ${LSP} Token`}
      subtitle={
        "Your token will be deployed on the blockchain (no confirmation needed). It will then be minted (you will need to confirm to mint)."
      }
      mainOverride={"rounded-tl-none"}
    >
      <div className="mb-4">
      ...
      <div className="mb-4 flex flex-row justify-between gap-2">
        ...
        <input
          ...
          className="hidden"
          type="file"
          onChange={handleUpdateFile("tokenIcon")}
        />
        ...
        <input
          ...
          className="hidden"
          type="file"
          onChange={handleUpdateFile("imageFront")}
        />
        ...
        <input
          ...
          className="hidden"
          type="file"
          onChange={handleUpdateFile("imageBack")}
        />
      </div>
      ...
      <div className="mb-4 flex h-8 flex-row items-center justify-between">
        <div className="font-semibold text-white">Token Color</div>
        <input
          type="color"
        ...
        ></input>
        <div className="ml-16 font-semibold text-white">Text Color</div>
        <input
          type="color"
        ...
        <div className="mb-4 flex flex-row">
          <input
            ...
            onChange={(e) =>
              setFormValues((current) => ({
                ...current,
                isNotDivisible: e.target.checked,
              }))
            }
          />
          <div className={inputLabel + " ml-1"}>
            Integer Token Supply (not used ATM)
          </div>{" "}
          {/* isNotDivisible */}
        </div>
      )}
