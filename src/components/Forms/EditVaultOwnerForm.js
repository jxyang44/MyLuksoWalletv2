...
import { createErc725Instance, LSP10Schema, UniversalProfileContract } from "../../utils/luksoConfigs";
...
const EditVaultOwnerForm = () => {
  const { web3Window, currentAccount, accountAddresses, fetchAddresses, profileJSONMetadata, useRelay, executeViaKeyManager } = useProfileContext();
  ...
  const handleRemoveVault = () => {
    ...
    swal(
      `Please confirm that you want to remove ${profileJSONMetadata["MLW_Vault_" + formValues.addressFrom]?.vaultName ?? "Unnamed Vault"} - ${
        formValues.addressFrom
      } from your Universal Profile.`,
      {
        buttons: true,
      }
    )
        ...
        if (value) {
          removeVault(value).then(res => {
            if (res) {
              swal(
                `Congratulations!,
                ${profileJSONMetadata["MLW_Vault_" + formValues.addressFrom]?.vaultName ?? "Unnamed Vault"} - ${
                  formValues.addressFrom
  ...
  async function removeVault(vaultAddress) {
    try {
      swal("Removing vault from Universal Profile...", `Vault address: ${vaultAddress}`, { button: false, closeOnClickOutside: true });
