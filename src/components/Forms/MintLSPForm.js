...
const MintLSPForm = ({ formValues, setFormValues, initialMintState, LSP }) => {
  ...
  useEffect(() => {
    ...
    supportsInterface(formValues.tokenAddress, LSPMapping[LSP].contract).then(
      (result) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!supportsInterfaceState)
      return swal(`Could not detect a ${LSP} interface for this asset.`);
    ...
    if (LSP === "LSP7") {
      if (
        ...
        return swal(`Please fill out all required fields.`);
      if (formValues.mintAmount <= 0)
        return swal(`Please enter a positive amount.`);
      mintLSP7(
        formValues.tokenAddress,
        formValues.mintAmount,
        formValues.mintToAddress,
      ...
      if (
        ...
        return swal(`Please fill out all required fields.`);
      if (mintToMessage !== `This Address is a Valid ERC725 Account`)
        return swal(`You cannot mint to this account.`);
  ...
  useEffect(() => {
    ...
    getAccountType(formValues.mintToAddress).then((res) => {
      if (res === "Invalid") setMintToMessage("This Address is Not Valid");
        ...
        setMintToMessage(
          "This Address is an EOA Account (minting to an EOA is not recommended or supported at this time)"
        );
  ...
  return (
    <FormContainer
      ...
      subtitle={`The contract must already be deployed and must support ${LSP}. Please make sure you have permissions to mint this token. ${
        LSP === "LSP8"
          ? "(The current implementation of mints identical properties to new token IDs. This will be updated in the future.)"
          : ""
      }`}
    ...
    >
        ...
        {formValues.tokenAddress !== "" && (
          <>
            {supportsInterfaceState ? (
              <div
                className={`${inputLabel} text-green-500`}
              >{`Valid ${LSP} address`}</div>
            ) : (
              <div
                className={`${inputLabel} text-red-500`}
              >{`Invalid ${LSP} address`}</div>
            )}
          </>
        )}
      ...
      <div
        className={`${inputLabel} ${
          mintToMessage === "This Address is a Valid ERC725 Account"
            ? "text-green-500"
            : "text-red-500"
        }`}
      >
