...
const MintForm = ({assetAddress}) => {
  ...
  const mintToken = () => {
      ...
      const mintFunction = async () => {
        try{
          ...
          const mint = await assetContract.methods.mint(currentAccount, amount, force, data).send({ from: currentAccount });
  ...
  return (
    <div className = "flex flex-col w-2/6 animate-fadeInLeft">
      ...
      <Input field="Mint Amount" placeholder="0" type="number" 
        value={mintAmount} setValue={setMintAmount}
        customFunc={mintToken} buttonDescription="Mint" 
        themeColor="from-green-500 to-green-600" themeText="text-green-700" />
    </div>
  )
}
