  ...
  const handleTransfer = () => {
    const transferInputs = async () => {
      const getAccountType = async address => {
        try {
          return await web3Provider.eth.getCode(address).then(res => {
            if (res === "0x") { //EOA/UP distinction currently not used
              return "EOA";
            } else {
              return "UP";
            }
      ...
      if (toAddress) {
        ...
        const { value: toAmount } = await Swal.fire({
          ...
          inputValidator: async value => { //error handling
    ...
    const transfer = async (toAddress, toAmount) => {
      try {
        ...
        if (useRelay) { //use key manager if relay function is enabled
          const myUP = new web3Window.eth.Contract(UniversalProfileContract.abi, currentAccount);
          return executeViaKeyManager(
            myUP.methods.execute(0, toAddress, web3Window.utils.toWei(toAmount), "0x").encodeABI,
            "Transferring funds via Key Manager..."
          );
        } else {  //transfer directly from account
