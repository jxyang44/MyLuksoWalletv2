...
const LYXBalanceFuncs = () => {
  ...
  const handleTransfer = () => {
    ...
    const transferInputs = async () => {
      //async function to guide user through transfer modals
      ...
      if (toAddress) {
        ...
        if (toAmount && toAddress && toAmount > 0 && toAmount <= accountBalance) {
          //if all inputs are valid, transfer LYX
          transfer(toAddress, toAmount).then(res => {
            console.log(res);
            if (res === undefined) swal("", "Something went wrong.", "warning");
            else {
              updateAccountBalance(currentAccount);
              swal(
                "Congratulations!",
                `${toAmount} LYX was successfully transferred from ${currentAccount} to ${toAddress}. Your remaining balance is ${accountBalance}.`,
                "success"
              );
            }
          });
    ...
    const transfer = async (toAddress, toAmount) => {
      try {
        ...
        swal("Fetching key manager address...", {
          button: false,
