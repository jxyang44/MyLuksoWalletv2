...
const ProfileTags = () => {
  ...
  return (
    <div className="flex flex-row text-sm mb-4 items-center justify-between text-center gap-1 border-2 rounded-2xl bg-slate-500 py-3 px-2">
                ...
                {
                  pendingProfileJSONMetadata.tags.length &&
                  <div className="flex flex-row flex-wrap justify-start gap-2 italic text-normal text-white w-[350px]">
                    {
                      pendingProfileJSONMetadata.tags.map((tag, index)=>{
                        return(
                        <div className='flex flex-row items-center gap-1' id={tag+index}>
                          <EditText
                            defaultValue={tag} 
                            ...
                            style={{borderTopLeftRadius: "9999px", borderBottomLeftRadius: "9999px", fontSize:'0.875rem', lineHeight:'0.875rem', minWidth:'1.25vmax', maxWidth:'16vmax', backgroundColor: '#9ca3af', padding: '0.375rem'}}
                          />
                ...
                
                {
                  ...
                  <button className='text-lg bg-[#9ca3af] p-1.5 rounded-full hover:text-blue-400' 
                    onClick={()=>
                      setPendingProfileJSONMetadata(current=>({...current, tags:[...current.tags,"new tag"]}))}>
                    <RiAddLine/>
                  </button>
                }
